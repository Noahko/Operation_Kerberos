...
If (GVAR(msg_cur) isEqualTo []) then {
    ...
    (_idd displayCtrl IDC_MSG_STRIPE) ctrlSetBackGroundColor _color;
    (_idd displayCtrl IDC_MSG_HEADER) ctrlSetText _title;
    (_idd displayCtrl IDC_MSG_TEXT) ctrlSetStructuredText (parseText _content);
    If (_title isEqualTo "") then {
        /// content & background
        private _curPos = ctrlPosition(_idd displayCtrl IDC_MSG_BACKGROUND);
        _curPos set [3,(ctrlPosition(_idd displayCtrl IDC_MSG_TEXT))select 2];
        (_idd displayCtrl IDC_MSG_BACKGROUND) ctrlSetPosition _curPos;
        (_idd displayCtrl IDC_MSG_BACKGROUND) ctrlCommit 0;
        (_idd displayCtrl IDC_MSG_TEXT) ctrlSetPosition _curPos;
        (_idd displayCtrl IDC_MSG_TEXT) ctrlCommit 0;
        /// color
        private _curPos = ctrlPosition(_idd displayCtrl IDC_MSG_STRIPE);
        _curPos set [3,(ctrlPosition(_idd displayCtrl IDC_MSG_TEXT))select 3];
        (_idd displayCtrl IDC_MSG_STRIPE) ctrlSetPosition _curPos;
        (_idd displayCtrl IDC_MSG_STRIPE) ctrlCommit 0;
    };
    If (_content isEqualTo "") then {
        /// background
        private _curPos = ctrlPosition(_idd displayCtrl IDC_MSG_BACKGROUND);
        _curPos set [3,(ctrlPosition(_idd displayCtrl IDC_MSG_HEADER))select 3];
        (_idd displayCtrl IDC_MSG_BACKGROUND) ctrlSetPosition _curPos;
        (_idd displayCtrl IDC_MSG_BACKGROUND) ctrlCommit 0;
        /// color
        private _curPos = ctrlPosition(_idd displayCtrl IDC_MSG_STRIPE);
        ...
        while {!(GVAR(msg_cur) isEqualTo [])} do {
            ...
            If !(GVAR(msg_waiting) isEqualTo []) then {
                ...
                /// pushdown the old ones;
                {
                    ...
                    }forEach [IDC_MSG_BACKGROUND,IDC_MSG_STRIPE,IDC_MSG_HEADER,IDC_MSG_TEXT];
